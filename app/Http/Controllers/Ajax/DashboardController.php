...
class DashboardController extends Controller
{
    ...
    public function findModelObject(Request $request){
        ...
        try {
            ...
            if (!$class) {
                return response()->json($this->findWidgetItemByQuery($model, $keyword, $this->language, $alias));
            }
            ...
            try {
                ...
                Log::warning('findWidgetItem failed, fallback query: ' . $e->getMessage());
            }

            return response()->json($this->findWidgetItemByQuery($model, $keyword, $this->language, $alias));
        } catch (\Throwable $e) {
            Log::error('findModelObject error: ' . $e->getMessage());
            return response()->json([]);
        }
    }

    private function findWidgetItemByQuery(string $model, string $keyword, int $languageId, string $alias): Collection
    {
        $table = Str::snake($model) . 's';
        $languageTable = $alias;

        $publishColumn = SchemaCache::hasColumn($table, 'publish') ? 'publish' : 'pubish';

        $query = DB::table($table . ' as o')
            ->join($languageTable . ' as ol', function ($join) use ($table, $model, $languageId) {
                $join->on('ol.' . $model . '_id', '=', 'o.id')
                    ->where('ol.language_id', '=', $languageId);
            })
            ->where('o.' . $publishColumn, 2)
            ->where('o.deleted_at', null)
            ->where('ol.name', 'LIKE', '%' . $keyword . '%')
            ->select([
                'o.id',
                'o.image',
                'o.icon',
        ...
        return $query->map(function ($item) use ($model) {
