...
class WidgetController extends Controller
{
    ...
    private function menuItemAgrument(array $whereIn = [], string $model = '')
    {
        $language = $this->language;
        $languageTable = Str::lower(Str::snake($model)) . '_language';
        return [
            'condition' => [],
            'flag' => true,
            'relation' => [
                'languages' => function ($query) use ($language, $languageTable) {
                    $query->where($languageTable . '.language_id',  $language)
                        ->select(
                            'languages.id as language_id',
                            $languageTable . '.name',
                            $languageTable . '.canonical',
                            $languageTable . '.description',
                            $languageTable . '.meta_title',
                            $languageTable . '.meta_keyword',
                            $languageTable . '.meta_description',
                            $languageTable . '.content'
                        );
                }
            ],
            'orderBy' => ['id', 'desc'],
    ...
    public function edit($id)
    {
        ...
        $widget->description = $widget?->description[$this->language] ?? 1;
        ...
        if ($widget->model_id != null) {
            $widgetItem = convertArrayByKey($modelClass->findByCondition(
                ...array_values($this->menuItemAgrument($widget->model_id, $widget->model))
