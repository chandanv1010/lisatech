...
class LanguageController extends Controller
{
    ...
    private function respositoryInstance($model){
        $subfolders = ['Core', 'Product', 'Post', 'User', 'Menu', 'Customer', 'Attribute'];
        $repositoryNamespace = '\App\Repositories\\' . ucfirst($model) . 'Repository';
        if (class_exists($repositoryNamespace)) {
            return app($repositoryNamespace);
        }
        foreach ($subfolders as $folder) {
            $repositoryNamespace = '\App\Repositories\\' . $folder . '\\' . ucfirst($model) . 'Repository';
            if (class_exists($repositoryNamespace)) {
                return app($repositoryNamespace);
            }
        }
