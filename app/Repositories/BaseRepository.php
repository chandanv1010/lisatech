...
class BaseRepository
{
    ...
    public function updateByWhere($condition = [], array $payload = []){
        ...
        foreach($condition as $key => $val){
            $val[0] = $this->normalizeColumn($val[0]);
    ...
    //             WHERE id IN (?)
    //             UNION ALL 
    //             SELECT c.id, c.parent_id, deleted_at
    //             FROM $table as c
    ...
    public function breadcrumb($catalogue, $language_id = 1){
        if (is_null($catalogue)) {
            return collect([]);
        }
        $tableName = $this->model->getTable();
        $languageTable = substr($tableName, 0, -1) . '_language'; 
        $foreignKey = substr($tableName, 0, -1) . '_id'; 
        ...
        return $this->model->select([
            ...
            ->join($languageTable . ' as tb2', 'tb2.' . $foreignKey, '=', $tableName . '.id')
            ->where('tb2.language_id', '=', $language_id)
            ->where($tableName . '.lft', '<=', $catalogue->lft)
            ->where($tableName . '.rgt', '>=', $catalogue->rgt)
