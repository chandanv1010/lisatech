...
class PostCatalogueService extends BaseService
{
    ...
    public function paginate($request, $languageId){
        ...
        $englishIds = $this->getDescendantIds('post_catalogues', 1056);
        $rawQuery = [];
        if ($languageId == 1) {
            if (!empty($englishIds)) {
                $rawQuery['whereRaw'] = [
                    ['post_catalogues.id NOT IN (' . implode(',', $englishIds) . ')', []]
                ];
            }
        } else {
            if (!empty($englishIds)) {
                $rawQuery['whereRaw'] = [
                    ['post_catalogues.id IN (' . implode(',', $englishIds) . ')', []]
                ];
            } else {
                $rawQuery['whereRaw'] = [
                    ['1 = 0', []]
    ...
    private function createCatalogue($request){
        $payload = $this->normalizeCataloguePayload($request->only($this->payload()));
