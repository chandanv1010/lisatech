...
class BaseService 
{
    ...
    public function getDescendantIds(string $table, int $parentId): array
    {
        $parentField = \Illuminate\Support\Facades\Schema::hasColumn($table, 'parent_id') ? 'parent_id' : 'parentid';
        $ids = [$parentId];
        $allIds = $ids;
        
        while (!empty($ids)) {
            $ids = DB::table($table)
                ->whereIn($parentField, $ids)
